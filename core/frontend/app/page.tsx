          ...
          <div className="text-center mb-16">
            ...
            >
              {(toolsByCategory[selectedCategory as keyof typeof toolsByCategory] || []).map((tool, index) => (
                ...
                >
                  {/* Enhanced image container with hover effects and improved rating badge */}
                  <div className="relative overflow-hidden">
                    {/* Premium badge - visible based on rating */}
                    {tool.rating >= 4.8 && (
                      <div className="absolute top-4 right-4 z-10 bg-gradient-to-r from-amber-500 to-yellow-500 text-white text-xs font-bold px-2.5 py-1 rounded-full shadow-md transform -rotate-3 group-hover:rotate-0 transition-all duration-300">
                        Premium
                      </div>
                    )}
                    
                    {/* Enhanced rating badge with animation */}
                    <div className="absolute top-4 left-4 z-10 bg-white/90 backdrop-blur-sm rounded-lg px-2.5 py-1.5 text-sm font-medium text-gray-900 flex items-center gap-1.5 shadow-sm group-hover:bg-white group-hover:shadow-md transition-all duration-300">
                      <Star className="w-4 h-4 text-yellow-500 fill-yellow-500 group-hover:scale-110 transition-transform duration-300" />
                      <span className="font-semibold">{tool.rating}</span>
                    </div>
                    
                    {/* Image with gradient overlay and enhanced hover effect */}
                  ...
                  <div className="p-5">
                    ...
                    {/* Improved name and price layout */}
                    ...
                    {/* Enhanced location with interactive feel */}
